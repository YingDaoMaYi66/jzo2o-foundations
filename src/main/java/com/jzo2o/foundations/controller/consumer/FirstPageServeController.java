...
public class FirstPageServeController {
    ...
    public List<ServeAggregationSimpleResDTO> hotServeList(@RequestParam("regionId") Long regionId) {
        return homeService.findHotServeAggregationByRegionId(regionId);
    }

    @GetMapping("/{id}")
    @ApiOperation("服务详情页面")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "serveid", required = true, dataTypeClass = Long.class)
    })
    public ServeAggregationSimpleResDTO serveDetail(@PathVariable("id") Long serveId) {
        return homeService.queryServeDetailByServeId(serveId);
    }
